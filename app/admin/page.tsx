...
export default function AdminPage() {
  ...
  const stats = [
    { label: "Total Users", value: users.length || "0", color: "from-blue-600 to-blue-700" },
    { label: "Verified Members", value: users.filter((u: any) => u.isVerified).length || "0", color: "from-green-600 to-green-700" },
    { label: "Pending Review", value: users.filter((u: any) => !u.isVerified).length || "0", color: "from-orange-600 to-orange-700" },
    { label: "Active Communities", value: "0", color: "from-purple-600 to-purple-700" },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat) => (
            <div key={stat.label} className={`bg-gradient-to-br ${stat.color} p-6 rounded-lg shadow-lg`}>
              <p className="text-slate-200 text-sm font-semibold uppercase tracking-wide">{stat.label}</p>
              <p className="text-4xl font-bold text-white mt-2">{stat.value}</p>
              <p className="text-slate-300 text-xs mt-2">Live data</p>
            </div>
          ))}
        </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-lg shadow-lg overflow-hidden">
          <div className="p-6 border-b border-slate-700">
            <h2 className="text-2xl font-bold text-white">Platform Users</h2>
            <p className="text-slate-400 text-sm mt-1">All registered community members</p>
          </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700 bg-slate-900/50">
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wide">User</th>
