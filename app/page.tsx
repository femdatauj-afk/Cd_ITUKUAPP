...
export default function Home() {
  return (
    <main className="landing-page">
      ...
      <section className="village-section" id="villages"><p className="eyebrow">OUR COMMUNITY</p><h2>Nine villages, beautifully connected.</h2><div className="villages">{villages.map((village, index) => <Link className="village" href={`/communities/${village.toLowerCase()}`} key={village} aria-label={`Open ${village} community`}><span>0{index + 1}</span>{village}</Link>)}</div></section>
      ...
      <footer><div className="brand"><Image src={logoImage} alt="ItukuApp logo" width={32} height={32} className="brand-logo" /><span>Ituku<span>App</span></span></div><p>One Community. Nine Villages. One Voice.</p><div className="footer-links"><Link href="/about">About</Link><Link href="/settings">Privacy</Link><Link href="/about">Terms</Link><a href="mailto:ITUKUAPP@GMAIL.COM">Support</a></div><small>© 2026 ItukuApp. Built with pride for Ituku Community.</small></footer>
    </main>
  );
}
