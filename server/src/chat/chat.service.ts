...
export class ChatService {
  ...
  async friends(userId: string, query = '') {
    const links = await this.prisma.friendship.findMany({
      ...
      include: { user: { select: person }, friend: { select: person } },
    });
    const needle = this.normalizeQuery(query);
    return links
      .map((link) => (link.userId === userId ? link.friend : link.user))
      .filter(
        (user) =>
          !needle ||
  ...
  async openDirect(userId: string, otherUserId: string) {
    ...
    if (!friend)
      throw new ForbiddenException(
        'You can only start conversations with accepted friends.',
      );
  ...
  async detail(userId: string, conversationId: string, cursor?: string) {
    ...
    return {
      ...
      nextCursor: messages.length > 50 ? messages[50].id : null,
    };
  }
  ...
  async send(
    ...
    const target = await this.prisma.conversationParticipant.findFirst({
      where: { conversationId: input.conversationId, userId: { not: userId } },
    });
    if (!target)
      throw new BadRequestException(
        'Direct conversation recipient is missing.',
      );
  ...
  async edit(userId: string, messageId: string, content: string) {
    ...
    if (
      ...
      throw new ForbiddenException('Only your text messages can be edited.');
  ...
  async remove(userId: string, messageId: string) {
    ...
    if (!message || message.senderId !== userId)
      throw new ForbiddenException('Only your messages can be deleted.');
    return this.prisma.message.update({
      where: { id: messageId },
  ...
  async startCall(userId: string, conversationId: string, type: CallType) {
    await this.assertMember(conversationId, userId);
    const recipient = await this.prisma.conversationParticipant.findFirst({
      where: { conversationId, userId: { not: userId } },
    });
    if (!recipient) throw new BadRequestException('Call recipient missing.');
    return this.prisma.call.create({
      ...
      include: { caller: { select: person } },
    });
  }
  async callAction(userId: string, callId: string, status: CallStatus) {
    const call = await this.prisma.call.findUnique({ where: { id: callId } });
    if (!call || (call.callerId !== userId && call.receiverId !== userId))
      throw new ForbiddenException('You cannot change this call.');
    const now = new Date();
    ...
    return this.prisma.call.update({
      ...
      data: {
        status,
        answeredAt: status === CallStatus.ACCEPTED ? now : call.answeredAt,
        endedAt: finalStatuses.includes(status) ? now : null,
        duration,
      },
    });
  }
